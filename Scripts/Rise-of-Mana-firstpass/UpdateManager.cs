...
public class UpdateManager : MonoBehaviour
{
	...
	public class DelegateEvent : UIEvent
	{
		private EventHandler method;

		private object param;

		public DelegateEvent(EventHandler method, object param)
		{
			this.method = method;
			this.param = param;
		}
	...
	private class SendMessageEvent : UIEvent
	{
		...
		public SendMessageEvent(GameObject _target, string _method, object _param)
		{
			target = _target;
			method = _method;
			param = _param;
		...
		public void Invoke()
		{
			if (!(target == null))
			{
				target.SendMessage(method, param, SendMessageOptions.DontRequireReceiver);
			}
		}
	}
	...
	private List<UIEvent> mEventQueue = new List<UIEvent>();
	...
	private void UpdateEvent()
	{
		...
		{
			UIEvent uIEvent = mEventQueue[0];
			mEventQueue.Remove(uIEvent);
			uIEvent.Invoke();
		}
	}
	...
	public static void AddButtonEvent(GameObject target, string method, object param)
	{
		if (!(target == null) && !string.IsNullOrEmpty(method))
		{
			SendMessageEvent sendMessageEvent = new SendMessageEvent(target, method, param);
			sendMessageEvent.Invoke();
		}
	}

	public static void AddDelegateEvent(EventHandler method, object param)
	{
		if (method != null)
		{
			DelegateEvent delegateEvent = new DelegateEvent(method, param);
