...
public sealed class FB : ScriptableObject
{
	...
	public abstract class RemoteFacebookLoader : MonoBehaviour
	{
		...
		public static IEnumerator LoadFacebookClass(string className, LoadedDllCallback callback)
		{
			...
			else if (!(typeof(FBComponentFactory).GetMethod("GetComponent").MakeGenericMethod(facebookClass).Invoke(null, new object[1] { IfNotExist.AddNew }) is IFacebook fb))
			{
	...
	public static string UserId => (facebook == null) ? string.Empty : facebook.UserId;

	public static string AccessToken => (facebook == null) ? string.Empty : facebook.AccessToken;

	public static DateTime AccessTokenExpiresAt => (facebook == null) ? DateTime.MinValue : facebook.AccessTokenExpiresAt;

	public static bool IsLoggedIn => facebook != null && facebook.IsLoggedIn;

	public static bool IsInitialized => facebook != null && facebook.IsInitialized;
