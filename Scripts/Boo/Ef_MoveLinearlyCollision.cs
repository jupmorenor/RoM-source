...
[Serializable]
public class Ef_MoveLinearlyCollision
{
	public bool active;
	...
	public Ef_MoveLinearlyCollision()
	{
		radius = 0.5f;
		sendColor = new Color(0f, 0f, 0f, 0f);
	}

	public void Update(Vector3 pos, Quaternion rot, Vector3 trgVec)
	{
		if (radius <= 0f)
		{
			return;
	...
	public void Emit(Vector3 pos, Quaternion rot)
	{
		...
		gameObject = ((!ajustRot) ? GameAssetModule.Instantiate(hitObj, pos, rot) : GameAssetModule.Instantiate(hitObj, pos, rot));
		if (!gameObject)
		{
			return;
		}
