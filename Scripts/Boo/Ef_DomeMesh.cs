...
[Serializable]
public class Ef_DomeMesh : Ef_Base
{
	public Color color;
	...
	public void Start()
	{
		...
		checked
		{
			...
			if (!(fadeIn + fadeOut <= life))
			{
				float num2 = fadeIn + fadeOut;
				fadeIn = life / num2 * fadeIn;
				fadeOut = life / num2 * fadeOut;
			}
			if (worldPos)
			{
	...
	public void LateUpdate()
	{
		...
		num13 = ((!(life <= fadeInEnd)) ? ((fstLife - life) / fadeIn) : ((life >= fadeOut) ? 1f : (life / fadeOut)));
