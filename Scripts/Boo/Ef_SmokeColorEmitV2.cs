...
[Serializable]
public class Ef_SmokeColorEmitV2 : MonoBehaviour
{
	public int smokeObjIndex;

	public int waterObjIndex;
	...
	public void Smoke(Color color)
	{
		...
		if (emitActive != null)
		{
			gameObject = emitActive.Emit(smokeObjIndex);
		}
		else if (emitManager != null)
		{
			gameObject = emitManager.Emit(smokeObjIndex);
		}
	...
	public void Water(Color color, float waterHeight)
	{
		...
		if (emitActive != null)
		{
			gameObject = emitActive.Emit(position, waterObjIndex);
		}
		else if (emitManager != null)
		{
			gameObject = emitManager.Emit(position, waterObjIndex);
		}
		gameObject.SendMessage("setColor", color, SendMessageOptions.DontRequireReceiver);
		Ef_ActiveOn component = gameObject.GetComponent<Ef_ActiveOn>();
		if (component != null)
