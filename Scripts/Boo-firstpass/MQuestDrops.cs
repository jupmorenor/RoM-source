...
[Serializable]
public class MQuestDrops : MerlinMaster
{
	...
	[NonSerialized]
	private static Dictionary<int, MQuestDrops> _0024mst_002489 = new Dictionary<int, MQuestDrops>();

	[NonSerialized]
	private static MQuestDrops[] All_cache;
	...
	public static MQuestDrops[] All
	{
		get
		{
			...
			if (All_cache == null)
			{
				MerlinMaster.GetHandler("MQuestDrops");
				MQuestDrops[] array = (MQuestDrops[])Builtins.array(typeof(MQuestDrops), _0024mst_002489.Values);
				if (array.Length > 0)
				{
					All_cache = array;
					result = All_cache;
	...
	public static MQuestDrops Get(int _id)
	{
		MerlinMaster.GetHandler("MQuestDrops");
		return (!_0024mst_002489.ContainsKey(_id)) ? null : _0024mst_002489[_id];
	}

	public static void Unload()
	{
		_0024mst_002489.Clear();
		All_cache = null;
	}

	public static MQuestDrops New(Hashtable data)
	{
		object result;
		...
		{
			MQuestDrops mQuestDrops = Create(data);
			if (!_0024mst_002489.ContainsKey(mQuestDrops.Id))
			{
				_0024mst_002489[mQuestDrops.Id] = mQuestDrops;
			}
			result = mQuestDrops;
		}
		return (MQuestDrops)result;
	}

	public static MQuestDrops New2(string[] keys, object[] vals)
	{
		object result;
	...
	public static MQuestDrops Entry(MQuestDrops mst, object id)
	{
		object result;
		if (mst != null)
		{
			mst.setField("Id", id);
			_0024mst_002489[RuntimeServices.UnboxInt32(id)] = mst;
			result = mst;
		}
		else
		{
	...
	public static void AddMst(MQuestDrops mst)
	{
		if (mst != null)
		{
			_0024mst_002489[mst.Id] = mst;
		}
	}

	public static MQuestDrops Create(Hashtable data)
	{
		MQuestDrops mQuestDrops = new MQuestDrops();
		MQuestDrops result;
		if (data == null)
		{
			result = mQuestDrops;
		...
		else
		{
			...
			while (enumerator.MoveNext())
			{
				...
				mQuestDrops.setField((string)obj, data[current]);
			}
			result = mQuestDrops;
		}
		return result;
